...
function runCli(args) {
  return new Promise((resolve, reject) => {
    execFile('npx', ['renaiss', ...args], { timeout: config.CLI_TIMEOUT_MS }, (err, stdout, stderr) => {
      if (err) return reject(new Error(`CLI error running "renaiss ${args.join(' ')}": ${stderr || err.message}`));
      try {
...
async function getPacks(opts = {}) {
  const args = ['packs', '--json'];
  if (opts.includeInactive) args.push('--include-inactive');
  const result = await runCli(args);
  return result.cardPacks || [];
}

module.exports = { getMarketplaceListings, getMarketplaceListingsRaw, getCardDetail, getPacks };
